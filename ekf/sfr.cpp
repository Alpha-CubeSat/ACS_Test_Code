...
namespace sfr {
...
#ifndef SIMULATOR
        HardwareSerial *serial = &Serial1; // Assign pointer to Serial1
#else
    ...
    namespace imu {
        // OP Codes 2200
        SFRField<bool> powered(false, 0x2200);
        SFRField<bool> dlink_written(false, 0x2201);
        SFRField<bool> report_ready(false, 0x2202);
        SFRField<uint8_t> power_setting((uint8_t)sensor_power_mode_type::on, 0x2203);
        SFRField<uint8_t> max_fragments(20, 0x2204);
        SFRField<uint16_t> mode((uint16_t)sensor_mode_type::init, 0x2205);
        SFRField<uint16_t> init_mode((uint16_t)sensor_init_mode_type::awaiting, 0x2206);
        SFRField<uint16_t> failed_times(0, 0x2207);
        SFRField<uint16_t> failed_limit(5, 0x2208);
