...
inline Eigen::Matrix<double, 6, 1> f(const Eigen::Matrix<double, 6, 1> &x) {
  ...
  dxdt << -x(2) * x(4) + x(1) * x(5), x(2) * x(3) - x(0) * x(5),
      -x(1) * x(3) + x(0) * x(4), -x(4) * x(5), x(3) * x(5), -x(3) * x(4) / 3;
  return dxdt;
}

inline Eigen::Matrix<double, 6, 1>
rk4_step(const Eigen::Matrix<double, 6, 1> &x, double step_size) {
  ...
  Eigen::Matrix<double, 6, 1> result =
      x + (1.0 / 6.0) * step_size * (k1 + 2 * k2 + 2 * k3 + k4);

  return result;
}

Eigen::VectorXd EKF::rk4(const Eigen::VectorXd &x_initial, double f_step_size,
...
void EKF::initialize(double delta_t, const Eigen::VectorXd &initial_state,
                     const Eigen::MatrixXd &initial_covariance,
                     const Eigen::MatrixXd &process_noise_covariance,
                     const Eigen::MatrixXd &Rd, const Eigen::MatrixXd &Hd) {
  state = initial_state;
  Z = initial_state;
...
void EKF::predict(const Eigen::MatrixXd &J_k_k) {
  ...
  // std::cout << "prediction:  " << state(0) / 1000000.0 << ", " << state(1) /
  // 1000000.0 << ", " << state(2) / 1000000.0 << ", " << state(3) << ", " <<
  // state(4) << ", " << state(5)<< "\n";
...
// void EKF::correct()
// {
//     Eigen::MatrixXd K_k1 = covariance * H_d.transpose() * (H_d * covariance *
//     H_d.transpose() + R_d).inverse();

//     covariance = covariance -  K_k1 * (H_d * covariance * H_d.transpose() +
//     R_d) * K_k1.transpose();

//     state = state +  K_k1 * (Z - H_d * state);

//     std::cout << "correction:  " << state(0) / 1000000.0 << ", " << state(1)
//     / 1000000.0 << ", " << state(2) / 1000000.0 << ", " << state(3) << ", "
//     << state(4) << ", " << state(5) << "\n";
// }
...
void EKF::correct() {
  Eigen::MatrixXd S =
      H_d * covariance * H_d.transpose() + R_d; // Innovation covariance
  Eigen::MatrixXd K_k1 =
      covariance * H_d.transpose() * S.inverse(); // Kalman gain
  ...
  Eigen::MatrixXd I =
      Eigen::MatrixXd::Identity(state.size(), state.size()); // Identity matrix
...
Eigen::MatrixXd EKF::CalculateJacobian() {
  ...
  J << 0, wz, -wy, 0, -Bz, By, -wz, 0, wx, Bz, 0, -Bx, wy, -wx, 0, -By, Bx, 0,
      0, 0, 0, 0, -wz, -wy, 0, 0, 0, wz, 0, wx, 0, 0, 0, -wx / 3, -wy / 3, 0;
